...
if [[ "${DISTRIB_ID}" = "linuxmint" ]]; then
    ...
    case ${LINUXMINT_VERSION} in
        ...
        ;;
        *)
            echo "ERROR: unsupported ${DISTRIB_ID} version (${LINUXMINT_VERSION})." >&2
    ...
    echo "Remapping ${DISTRIB_ID} version ${LINUXMINT_VERSION} to ubuntu version ${DISTRIB_RELEASE}" >&2
fi
case $DISTRIB_ID in
    ubuntu|linuxmint)
        case $DISTRIB_RELEASE in
            ...
            *)
                ...
                echo "For additional versions open an issue or pull request at: https://github.com/powershell/powershell" >&2                exit 1
            ;;
        esac
    ;;
    debian)
